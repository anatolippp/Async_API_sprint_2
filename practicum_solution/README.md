...
* `scripts/generate_data.py` — наполняет MongoDB и PostgreSQL случайными лайками, рецензиями и закладками. Параметры `--records`/`DATASET_RECORDS` и `--batch`/`DATASET_BATCH` задают объём и размер пачки (по умолчанию 10_000_000 и 5000), генерация стримовая и использует фиксированный seed (`DATASET_SEED`), поэтому наборы данных идентичны в обеих СУБД; индексы по `user_id` и `movie_id` создаются автоматически.
...
Результаты последнего прогона на 10 млн лайков/1 млн закладок/2 млн рецензий находятся в `reports/performance.md` и укладываются в бюджет 200 мс для всех сценариев чтения.
