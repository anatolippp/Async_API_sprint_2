...
4. Подготовьте переменные в `.env` **до запуска контейнеров**:
   ...
   - Остальные поля (`MONGO_URL`, `POSTGRES_DSN`, `DATASET_RECORDS`, `DATASET_BATCH`, `DATASET_SEED`) можно оставить как в шаблоне или поменять под ресурсы машины.
5. Если машина слабая, уменьшите объём генерации, выставив `DATASET_RECORDS` (например, 1_000_000 вместо 10_000_000) и размер пачки `DATASET_BATCH` (например, 2000) — это уменьшит время сидирования.
6. Проверьте, что `MONGO_URL` и `POSTGRES_DSN` вас устраивают; по умолчанию они смотрят на контейнеры `mongo` и `postgres`, поднятые в том же compose.
...
9. Следите за прогрессом в логах сервисов `seed` и `benchmark`: `docker compose logs -f seed benchmark` — сидирование завершится сообщением об успешной вставке, затем стартует бенчмарк.
10. После окончания бенчмарка загляните в `reports/performance.md` (на хосте), чтобы увидеть средние и p95 задержки по сценариям чтения; файл создаётся или перезаписывается автоматически контейнером `benchmark`.
